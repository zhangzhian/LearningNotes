...
内核是操作系统的核心部分，为应用程序提供安全访问硬件资源的功能。可以被看作是一个系统管理器，内核管理计算机系统中所有软件和硬件资源

Linux内核代码的官方站点：https://www.kernel.org/

### Linux内核的子系统

用户程序通过软件中断后，调用系统内核提供的功能，这个在用户空间和内核提供服务之间的接口被称为系统调用（SCI）
...
Linux系统启动的流程分为3部分：固话在硬件的程序、内核代码部分和用户空间的程序和脚本

内核代码可分为2部分：依赖体系结构的代码和体系无关代码

### PC的初始化流程

打开电源--BIOS--GRUB/LILO--内核--init进程--mingetty--shell--进入用户界面

### 嵌入式的初始化程序

上电--设置中断向量--配置系统存储器--初始化看门狗和外围电路--初始化存储器--初始化堆栈指针--进入Bootloader业务流程--加载Linux内核映像--启动Linux内核--进入Linux系统
...
在Linux内核中，对网络部分按照网络协议层，网络设备层，设备驱动层和网络媒介层的分层体系设计

## Flash设备驱动

Linux内核设计了一个MTD（内存技术设备）结构支持Flash设备，用户需要按照MTD的要求设置Flash设备的参数并且提供驱动。

内核MTd从上到下可以分为：设备节点，MTD设备层，MTD原始设备层和硬件驱动层

NAND和NOR是两种不同的Flash存储技术，性能比较：

- NAND的写入速度高于NOR
- NOR的读速度高于NAND
- NAND的擦速度远高于NOR
- NAND的擦单元小，需要擦的电路也会减小

## USB驱动开发

USB是一个总线协议标准，USB总线提供了所有外部设备的统一连接方式，支持热拔插，方便厂商开发设备和用户使用设备

一个USB系统可以分为：USB互联，USB设备和USB主机3个部分
